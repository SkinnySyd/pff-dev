...
public class ServerResource {
    ...
    public ResponseEntity<Response> getServers() {
        System.out.println(" " + ServerResource.class.getNestMembers());
    ...
    @PostMapping("/{serverId}/reset/{ipAddr}")
    public ResponseEntity<String> resetServer(@PathVariable String ipAddr, @PathVariable Long serverId) {
        try {
            serverService.shutdownWindowsServer(ipAddr, serverId);
            return ResponseEntity.ok("Server reset successfully");
        } catch (Exception e) {
            return ResponseEntity.status(500).body("Error resetting server: " + e.getMessage());
        }
    }

    @PostMapping("/{serverId}/restart")
    public ResponseEntity<String> restartServer(@PathVariable String ipAddr, @PathVariable Long serverId) {
        try {
            serverService.shutdownWindowsServer(ipAddr, serverId);
            return ResponseEntity.ok("Server restarted successfully");
        } catch (Exception e) {
            return ResponseEntity.status(500).body("Error restarting server: " + e.getMessage());
        }
    }
}
